content = r'''边界值测试可以适用于function(method) level，class level，system level，分析电商平台系统测试时考虑的边界值情况。
'''

answer = r'''对于电商平台，最重要的流程就是购物流程，包括几个重要功能：个人信息、购物车、支付方式、提交订单。我们讨论时以功能性测试为主，考虑以下功能：
'''
...
项目代码中，类之间和模块间的接口测试等,如用户订单和商品之间的接口，考虑订单一开始没有商品的情况。

**Method Level**

对个方法进行单独测试，检查参数的边界值情况。例如购买某个商品的方法，可能需要考虑余额不足的情况。
